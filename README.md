...
**2026-07-10** and **2026-09-07**: `438,357,177` quote observations across
...
**GitHub Releases** (a single dated tarball, ~208 MB):

```bash
gh release download data-2026-09-08 --repo DineshKumar8399/polymarket-orderbook-dataset
tar --zstd -xf polymarket-orderbook-*.tar.zst
```

Each release is a frozen snapshot, so `data-2026-09-08` is reproducible: cite the
...
|---|---|---|
| `quotes/dt=YYYY-MM-DD/*.parquet` | `438,357,177` | Book quotes for every tracked market, partitioned by date |
| `markets.parquet` | `249,473` | One row per market: question text, category, coverage |
| `labels.parquet` | `176,940` | Binary settlement outcomes, with a `source` column |
| `watch_quotes.parquet` | `2,626,511` | High-frequency feed — **the only table with traded prices** |
| `data_quality.parquet` | 7 | The known issues below, as queryable rows |

Total: about `208 MB` of ZSTD-compressed Parquet.
...
`438,357,177` times is most of why the raw CSV was 32 GB.
...
covers 6,601 markets rather than 249,473. Note `last_traded` is itself 83.4%
populated, not 100%.
