...
**2026-07-10** and **2026-09-06**: `429,732,177` quote observations across
`249,473` markets, plus settlement outcomes and a separate high-frequency feed
...
**GitHub Releases** (a single dated tarball, ~206 MB):

```bash
gh release download data-2026-09-07 --repo DineshKumar8399/polymarket-orderbook-dataset
tar --zstd -xf polymarket-orderbook-*.tar.zst
```

Each release is a frozen snapshot, so `data-2026-09-07` is reproducible: cite the
...
|---|---|---|
| `quotes/dt=YYYY-MM-DD/*.parquet` | `429,732,177` | Book quotes for every tracked market, partitioned by date |
| `markets.parquet` | `249,473` | One row per market: question text, category, coverage |
| `labels.parquet` | `176,562` | Binary settlement outcomes, with a `source` column |
| `watch_quotes.parquet` | `2,576,327` | High-frequency feed — **the only table with traded prices** |
| `data_quality.parquet` | 7 | The known issues below, as queryable rows |

Total: about `206 MB` of ZSTD-compressed Parquet.
...
`429,732,177` times is most of why the raw CSV was 32 GB.
...
covers 6,585 markets rather than 249,473. Note `last_traded` is itself 83.5%
populated, not 100%.
...
markets (68%) carry an authoritative label, and they are **not a random 68%**.
