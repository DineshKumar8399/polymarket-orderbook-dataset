...
**2026-07-10** and **2026-08-25**: `332,880,379` quote observations across
...
**GitHub Releases** (a single dated tarball, ~170 MB):

```bash
gh release download data-2026-08-26 --repo DineshKumar8399/polymarket-orderbook-dataset
tar --zstd -xf polymarket-orderbook-*.tar.zst
```

Each release is a frozen snapshot, so `data-2026-08-26` is reproducible: cite the
...
|---|---|---|
| `quotes/dt=YYYY-MM-DD/*.parquet` | `332,880,379` | Book quotes for every tracked market, partitioned by date |
| `markets.parquet` | `190,813` | One row per market: question text, category, coverage |
| `labels.parquet` | `157,856` | Binary settlement outcomes, with a `source` column |
| `watch_quotes.parquet` | `1,988,945` | High-frequency feed — **the only table with traded prices** |
| `data_quality.parquet` | 7 | The known issues below, as queryable rows |

Total: about `170 MB` of ZSTD-compressed Parquet.
...
`332,880,379` times is most of why the raw CSV was 32 GB.
...
covers 5,640 markets rather than 190,813. Note `last_traded` is itself 83.7%
populated, not 100%.
...
So "has a label" correlates strongly with "settled before Jul 24" — 155,720
