...
**2026-07-10** and **2026-08-28**: `356,182,835` quote observations across
...
**GitHub Releases** (a single dated tarball, ~179 MB):

```bash
gh release download data-2026-08-29 --repo DineshKumar8399/polymarket-orderbook-dataset
tar --zstd -xf polymarket-orderbook-*.tar.zst
```

Each release is a frozen snapshot, so `data-2026-08-29` is reproducible: cite the
...
|---|---|---|
| `quotes/dt=YYYY-MM-DD/*.parquet` | `356,182,835` | Book quotes for every tracked market, partitioned by date |
| `markets.parquet` | `202,590` | One row per market: question text, category, coverage |
| `labels.parquet` | `158,934` | Binary settlement outcomes, with a `source` column |
| `watch_quotes.parquet` | `2,136,274` | High-frequency feed — **the only table with traded prices** |
| `data_quality.parquet` | 7 | The known issues below, as queryable rows |

Total: about `179 MB` of ZSTD-compressed Parquet.
...
`356,182,835` times is most of why the raw CSV was 32 GB.
...
covers 5,899 markets rather than 202,590. Note `last_traded` is itself 83.5%
populated, not 100%.
...
So "has a label" correlates strongly with "settled before Jul 24" — 156,780
